...
class HiddenRoomWorld(HiddenMiniWorldEnv):
    ...
    def calculate_reward(self, point_cloud: np.ndarray) -> float:
        """
        Calculates the reward for the given point cloud.

        Parameters:
        - point_cloud (np.ndarray): Point cloud of the environment.

        Returns:
        - reward (float): Reward for the given point cloud.
        - new_seen_area (float): New seen area.
        - completed (bool): Whether the episode is completed.
        """
